...
#include "misc/cost_function.hpp"
...
/// callback function
template<class Ntk>
using rebalance_function_callback_t = std::function<void(mockturtle::arrival_time_pair<Ntk> const&, uint32_t)>;

template<class Ntk>
using rebalance_function_t = std::function<void(Ntk&, kitty::dynamic_truth_table const&, std::vector<mockturtle::arrival_time_pair<Ntk>> const&, uint32_t, uint32_t, rebalance_function_callback_t<Ntk> const&)>;
...
public:
  ...
  {
    Ntk dest;
    mockturtle::node_map<mockturtle::arrival_time_pair<Ntk>, Ntk> old_to_new( _ntk );
    ...
    std::shared_ptr<mockturtle::depth_view<Ntk, CostFn>> depth_ntk;
    if ( _ps.only_on_critical_path )
    {
      depth_ntk = std::make_shared<mockturtle::depth_view<Ntk, CostFn>>( _ntk );
    }
    mockturtle::topo_view<Ntk>(_ntk).foreach_node([&](auto n){
      ...
      else
      {
        ...
        mockturtle::arrival_time_pair<Ntk> best{{}, std::numeric_limits<uint32_t>::max()};
        uint32_t best_size{};
        for ( auto& cut : _cut_network.cuts( index_node ) )
        {
          ...
          std::vector<mockturtle::arrival_time_pair<Ntk>> arrival_times( cut->size() );
          std::transform( cut->begin(), cut->end(), arrival_times.begin(), [&]( auto leaf ) { return old_to_new[_ntk.index_to_node( leaf )]; });

          _rebalancing_fn( dest, _cut_network.truth_table( *cut ), arrival_times, best.level, best_size, [&]( mockturtle::arrival_time_pair<Ntk> const& cand, uint32_t cand_size ) {
