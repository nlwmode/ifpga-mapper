...
format_fanin( Ntk const& ntk, node<Ntk> const& n, mockturtle::node_map<std::string, Ntk>& node_names )
{
...
void write_verilog( Ntk const& ntk, std::ostream& os, write_verilog_params const& ps = {} )
{
  ...
  mockturtle::node_map<std::string, Ntk> node_names( ntk );
  ...
  ntk_topo.foreach_node( [&]( auto const& n ) {
    ...
    {
      if constexpr ( mockturtle::has_is_nary_and_v<Ntk> )
      {
      ...
      if constexpr ( mockturtle::has_is_nary_or_v<Ntk> )
      {
        if ( ntk.is_nary_or( n ) )
        {
          writer.on_assign( node_names[n], detail::format_fanin<Ntk>( ntk, n, node_names ), "|" );
      ...
      if constexpr ( mockturtle::has_is_nary_xor_v<Ntk> )
      {
