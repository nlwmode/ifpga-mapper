...
#pragma region Node and signal iterators
  ...
  {
    mockturtle::detail::foreach_element( _storage->inputs.begin(), _storage->inputs.begin() + _storage->data.num_pis, fn );
  }

  template<typename Fn>
  void foreach_po( Fn&& fn ) const
  {
    mockturtle::detail::foreach_element( _storage->outputs.begin(), _storage->outputs.begin() + _storage->data.num_pos, fn );
  }

  template<typename Fn>
  void foreach_ro( Fn&& fn ) const
  {
    mockturtle::detail::foreach_element( _storage->inputs.begin() + _storage->data.num_pis, _storage->inputs.end(), fn );
  }

  template<typename Fn>
  void foreach_ri( Fn&& fn ) const
  {
    mockturtle::detail::foreach_element( _storage->outputs.begin() + _storage->data.num_pos, _storage->outputs.end(), fn );
  }

  template<typename Fn>
  void foreach_register( Fn&& fn ) const
  {
    static_assert( mockturtle::detail::is_callable_with_index_v<Fn, std::pair<signal,node>, void> ||
                   mockturtle::detail::is_callable_without_index_v<Fn, std::pair<signal,node>, void> ||
                   mockturtle::detail::is_callable_with_index_v<Fn, std::pair<signal,node>, bool> ||
                   mockturtle::detail::is_callable_without_index_v<Fn, std::pair<signal,node>, bool> );
    ...
    if constexpr ( mockturtle::detail::is_callable_without_index_v<Fn, std::pair<signal,node>, bool> )
    {
    ...
    else if constexpr ( mockturtle::detail::is_callable_with_index_v<Fn, std::pair<signal,node>, bool> )
    {
      uint32_t index{0};
      while ( ro != _storage->inputs.end() && ri != _storage->outputs.end() )
      {
    ...
    else if constexpr( mockturtle::detail::is_callable_without_index_v<Fn, std::pair<signal,node>, void> )
    {
      while ( ro != _storage->inputs.end() && ri != _storage->outputs.end() )
      {
    ...
    else if constexpr ( mockturtle::detail::is_callable_with_index_v<Fn, std::pair<signal,node>, void> )
    {
      uint32_t index{0};
      while ( ro != _storage->inputs.end() && ri != _storage->outputs.end() )
      {
  ...
  {
    auto r = mockturtle::range<uint64_t>( 1u, _storage->nodes.size() ); /* start from 1 to avoid constant */
    mockturtle::detail::foreach_element_if( r.begin(), r.end(),
...
#pragma region Value simulation
  ...
  {
    static_assert( mockturtle::iterates_over_v<Iterator, kitty::partial_truth_table>, "begin and end have to iterate over partial_truth_tables" );
